...
def create_enemy():
    ...
    enemy_rect = pygame.Rect(WIDTH, random.randint(0 + enemy_size[0], HEIGHT - enemy_size[0]), *enemy_size)
...
def create_bonus():
    ...
    bonus_rect = pygame.Rect(random.randint(0 + bonus_size[1], WIDTH - bonus_size[1]), 0, *bonus_size)
...
while playing:
    ...
    for event in pygame.event.get():
        ...
        if event.type == CHANGE_IMG:
            player = pygame.transform.scale(pygame.image.load(os.path.join(IMG_PATH, PLAYER_IMGS[img_index])), player_size)
